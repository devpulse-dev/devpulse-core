...
5. **Сбор per-repo sequential.** Виртуальные потоки — только для I/O внутри одной операции (git stdout reader). Параллелизм по репозиториям сознательно НЕ делаем — две параллельные tx в БД пересекались бы при cleanup + recompute, и distributed lock защищает один-в-один. См. ADR ниже.
