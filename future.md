...
**3. Cycle-time — `kaiten.cycleTime`**
```
{ medianDays: number|null, meanDays: number|null, count: int }
```
- Время от «в работу» до «готово» по закрытым в периоде карточкам. `count === 0` → `medianDays/meanDays = null` → показать «нет данных».
- UI: две плитки «медиана / среднее, дней» (+ подпись `по count карточкам`). Медиана честнее среднего (выбросы) — её крупнее.
